...
const Deal = () => {
    ...
    const updateView = async () => {
        ...
        try {
            const sellerAddress = await dealContract.methods.sellerAddr().call();
            if (address === sellerAddress) {
                getOrdersHandler();
                getOrders();
                return setSeller(true);
            }
            const buyerAddress = await dealContract.methods.buyerAddr().call();
    ...
    const getOrdersHandler = async () => {
        const orders = await dealContract.methods.orderCount().call();
        setOrders(orders);
    };
    ...
    const placeOrderHandler = async () => {
        try {
            await dealContract.methods.placeOrder(item, amount, "some courier").send(
                {
                    from: address,
